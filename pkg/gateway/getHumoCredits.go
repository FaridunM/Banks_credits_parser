...
func GetHumoStandartCredit(creditType, url string, prompts config.CreditsPrompts) (credit structs.Credit) {
	...
	if strings.Contains("agricultural, livestock", creditType) {
		coli.OnHTML(prompts.Collateral, func(h *colly.HTMLElement) {
			if ok, err := regexp.MatchString("[а-я ]:[а-я ]", h.DOM.Children().Text()); err == nil && ok {
				tempCollateral := h.DOM.Children().Text()
				tempCollateral = strings.Replace(tempCollateral, "\n\t", "", -1)
				tempCollateral = strings.Replace(tempCollateral, ".", "", -1)

				credit.Collateral = strings.Split(tempCollateral, ";")
			}
		})
	} else {
		coli.OnHTML(prompts.Collateral, func(h *colly.HTMLElement) {
	...
	if strings.Contains("education, agricultural, livestock", creditType) {
		coli.OnHTML(prompts.Amount, func(h *colly.HTMLElement) {
			if ok, err := regexp.MatchString("[0-9]{3,}", strings.Replace(h.Text, " ", "", -1)); err == nil && ok {
				...
				tempAmount := strings.ReplaceAll(h.DOM.Children().Text(), replaceElement, "")
				tempAmount = strings.ReplaceAll(tempAmount, "\n\t", "")
				credit.Amount = strings.Split(tempAmount, ";")
			}
		})
	} else {
		coli.OnHTML(prompts.Amount, func(h *colly.HTMLElement) {
			if ok, err := regexp.MatchString("[0-9]{3,}", strings.Replace(h.Text, " ", "", -1)); err == nil && ok {
				pattern := utils.TernarOp(creditType == "start_business", "(Сумма):(.*)", "(Сумма кредита|Сумма):([а-яА-Я0-9, ]*)")
				regex := regexp.MustCompile(pattern)
				tempStr := strings.TrimSpace(regex.FindAllStringSubmatch(strings.ReplaceAll(h.Text, ".", ""), -1)[0][2])
	...
	coli.OnHTML(prompts.ItinialPayment, func(h *colly.HTMLElement) {
	...
	case "agricultural", "livestock":
		...
		coli.OnHTML(prompts.LoanTerms, func(h *colly.HTMLElement) {
			if ok, err := regexp.MatchString("(Физические лица старше 20 лет|Количество заемщиков группы от 2 до 10 человек)", h.Text); err == nil && ok {
				...
				allTerms = strings.Split(strings.TrimSpace(strings.ReplaceAll(h.Text, "\n\t", "")), ";")
	...
	default:
		coli.OnHTML(prompts.LoanTerms, func(h *colly.HTMLElement) {
			if ok, err := regexp.MatchString("Физические лица старше 20 лет", h.Text); err == nil && ok {
				h.Text = strings.ReplaceAll(h.Text, ".", "")
				for _, value := range strings.Split(strings.ReplaceAll(h.Text, "\n\t", ""), ";") {
					value = strings.TrimSpace(value)
					credit.LoanTerms = append(credit.LoanTerms, value)
				}
			}
		})
	}
	...
	coli.OnHTML(prompts.NeedDocuments, func(h *colly.HTMLElement) {
		if ok, err := regexp.MatchString("Кредитная заявка", h.Text); err == nil && ok {
			h.Text = strings.ReplaceAll(h.Text, ".", "")
			tempNeedDocuments := strings.ReplaceAll(h.Text, "\n\t", "")
			tempNeedDocuments = tempNeedDocuments[:len(tempNeedDocuments)-1]
			credit.NeedDocuments = strings.Split(tempNeedDocuments, ";")
		}
	})
	credit.Url = url
...
func GetHumoOrzuCredit(url string) (credit structs.Credit) {
	defer utils.PanicCatcher()
	coli := func() *colly.Collector {
		return colly.NewCollector(
